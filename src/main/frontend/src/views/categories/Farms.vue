<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="Owner" align="center" width="90">
      ...
      <el-table-column label="Address" align="center" width="100">
        <template slot-scope="{row}">
      ...
      <el-table-column label="Last Update" align="center" width="150">
        <template slot-scope="{row}">
      ...
      <el-table-column fixed="right" label="Actions" align="center" width="210" class-name="small-padding fixed-width">
        <template slot-scope="{row,$index}">
          <el-button v-if="row.status" type="warning" size="mini">
            <router-link :to="{ name: 'devices', params: { owner: row.name }}" style="color:black">
              Devices
            </router-link>
          </el-button>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" center>
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="130px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item v-show="dialogStatus === 'update'" label="Owner" prop="user.username">
          <el-input v-model="temp.user.username" :disabled="true" />
        </el-form-item>
        <el-form-item v-show="dialogStatus === 'create'" label="Owner" prop="user">
          <el-autocomplete
            v-model="temp.user.username"
            clearable
        ...
        <el-form-item label="Address" prop="address">
          <el-input v-model="temp.address" @change.native="changeName" />
        </el-form-item>
...
<script>
import { getFarms, updateFarms, createFarms, deleteFarms } from '@/api/farm'
...
export default {
  ...
  methods: {
    ...
    handleSelect(item) {
      this.temp.user = item.key
    },
    changeName() {
      this.temp.name = this.temp.user.username + '(' + this.temp.address + ')'
    },
    formatDate(d) {
      d = new Date(d)
    ...
    getList() {
      ...
      getFarms(this.listQuery).then(response => {
    ...
    handleDelete(row, index, status) {
      deleteFarms(row.id, status).then(() => {
        if (!status) {
          this.$notify({
            title: 'Success',
